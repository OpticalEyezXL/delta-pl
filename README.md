...
| 파일 | 내용 |
|---|---|
| `physical_law_verification.py` / `physical_law_verification_en.py` | 다음을 검증한다: T1/T2(소산 반군의 시간적분과 d=3 뉴턴/쿨롱 그린함수), §3.1(V_mn(t)이 실수축 크기와 허수축 위상으로 분리됨), §10(완화-결어긋남 부등식, 2450개 모드쌍 전수검사), 부록K/T7(다체중력을 그린함수 중첩으로 — 수렴성까지 확인한 유한차분 라플라시안으로 검증), 그리고 (Δ 구조 자체에서 나온 결과가 아니라 **표준 에너지법을 이용한 부수적 탐구**이며, 나비에-스토크스 밀레니엄 문제와는 무관한) 이류항이 만드는 삼중결합텐서의 에너지중립성(§6 부기). |
